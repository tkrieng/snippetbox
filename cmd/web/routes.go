...
func (app *application) routes() http.Handler {
	...
	mux.Get("/snippet/create", app.session.Enable(noSurf(app.requireAuthentication(http.HandlerFunc(app.createSnippetForm)))))
	mux.Post("/snippet/create", app.session.Enable(noSurf(app.requireAuthentication(http.HandlerFunc(app.createSnippet)))))
	...
	mux.Post("/user/logout", app.session.Enable(noSurf(app.requireAuthentication(http.HandlerFunc(app.logoutUser)))))
